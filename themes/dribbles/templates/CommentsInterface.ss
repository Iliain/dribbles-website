<br>
<% if $CommentsEnabled %>
	<div id="$CommentHolderID" style="display: none;">
		<h4 class="title is-4">What do you think?</h4>
		...
		<% end_if %>
		<br>
		<h4 class="title is-4"><% _t('CommentsInterface_ss.COMMENTS','Comments') %></h4>
...
<%--SUUUUUUUPER hacky fix. CommentForm template is rather difficult to change--%>
<script>
    $(document).ready( function() {
        $('#CommentForm_CommentsForm_Name').addClass('input');
        $('#CommentForm_CommentsForm_Email').addClass('input');
        $('#CommentForm_CommentsForm_URL').addClass('input');
        $('#CommentForm_CommentsForm_Comment_Holder').removeClass('textarea');
        $('#CommentForm_CommentsForm_action_doPostComment').addClass('is-info');
        $('#$CommentHolderID').show();
    });
</script>
<style>
    fieldset {
